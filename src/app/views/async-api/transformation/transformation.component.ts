...
} from "~/app/utils/errors/validation-error";
import {
  ButtonType,
  GenericDialog,
} from "~/app/utils/generic-dialog/generic-dialog.component";
import {
  getOperationTemplates,
  getMessageSchema,
  IAsyncApiOperationTemplate,
} from "~/app/utils/asyncapi-parser";
...
export class TransformationComponent implements OnInit, OnDestroy {
  ...
  public async ngOnInit() {
    ...
    this.subscriptions.push(
      this.inputForm.valueChanges.pipe(pairwise()).subscribe(([prev, next]) => {
        if (prev.strict !== next.strict) {
          return; // Ignore updates of strict mode
        }
        this.initializeMapping();
      })
    );

    const sourceChanges = merge(
      this.inputForm.get("mS").valueChanges,
      this.inputForm.get("mSO").valueChanges
    ).pipe(debounceTime(0));
    this.subscriptions.push(
      sourceChanges.subscribe(async () => {
        ...
        this.sourceMessageBody = {
          [`${source.api.id}_${source.operationId}`]: await getMessageSchema(
            source.api,
    ...
    this.subscriptions.push(
      this.inputForm
        .get("mS")
        .valueChanges.subscribe(async (val: IAsyncApi) => {
          const { servers, operationTemplates } =
            val &&
            (await getOperationTemplates(
              val,
              this.inputForm.get("publish").value
            ));
    ...
    this.subscriptions.push(
      this.targets.valueChanges.subscribe(async () => {
        ...
        for (const [key, { api, operationId }] of Object.entries(
          targets || {}
        )) {
    ...
    this.subscriptions.push(
      this.inputForm
        .get("publish")
        .valueChanges.subscribe(async (publish: boolean) => {
          const val = this.inputForm.get("mS").value;
          const { servers, operationTemplates } =
            val && (await getOperationTemplates(val, publish));
  ...
  private parseSource(): IAsyncApiInterface {
    if (!(this.inputForm.get("mS").valid && this.inputForm.get("mSO").valid))
      return undefined;
  ...
  private parseTargets(): { [key: string]: IAsyncApiInterface } {
    ...
    const targets = this.targets.value as Array<{
      mT: IAsyncApi;
      mTServ: string;
      mTO: IAsyncApiOperationTemplate;
    }>;
  ...
  changeStrictMode(strict: boolean) {
    this.inputForm.get("strict").setValue(strict);
    ...
    for (const amP of this.automaticMappingPairs) {
      ...
      const idx = this.mappingPairs.findIndex((mP) =>
        arrayEquals(amP.required, mP.required)
      );
      if (idx !== -1) {
    ...
    this.mappingPairs = this.mappingPairs.filter(
      (mP) =>
        mP.creationType === MappingPairType.MANUAL ||
        this.automaticMappingPairs.includes(mP)
    );
  }
  ...
  private async getOperationSuggestions() {
    ...
    const targetIds = targets.reduce(
      (arr, target) => (target?.mT ? [...arr, target.mT.id] : arr),
      []
    );
    const direction = this.inputForm.get("publish").value
      ? MappingDirection.OUTPUT
      : MappingDirection.INPUT;
    const predictions = await this.mappingService.buildAsyncApiOperationPredictions(
      this.mappingSource,
      targetIds,
      direction
    );
    ...
    const predictionsByTargets = predictions.reduce<{
    ...
    for (const [apiId, apiPredictions] of Object.entries(
      predictionsByTargets
    )) {
      for (const target of this.targets.controls) {
        ...
        const operations: IAsyncApiOperationTemplate[] = target.get(
          "operations"
        ).value;
  ...
  mapSame() {
    if (this.inputForm.get("publish").value) {
      //Output
      const mappingPairs = this.mappingService.buildSameMappingPairs(
        this.sourceMessageBody,
        this.targetMessageBodies
      );
      mappingPairs.forEach((p) => {
        if (
          !this.mappingPairs.find(
            (e) => e.required.join(".") === p.required.join(".")
          )
        ) {
      ...
      mappingPairs.forEach((p) => {
        if (
          !this.mappingPairs.find(
            (e) =>
              e.required.join(".") === p.required.join(".") &&
              e.provided[0][0] === p.provided[0][0]
          )
        ) {
          this.mappingPairs.push(p);
        }
    ...
    try {
      if (this.mappingPairs.some((mp) => !mp.mappingCode)) {
        throw new AsyncApiValidationError(
          "Please enter a mapping code for the mappings marked in red (by clicking on it)"
        );
      }
      ...
      const source = this.parseSource();
      const targets = this.parseTargets();
      const mapping = this.mappingService.buildAsyncApiMapping(
        source,
        targets,
        this.mappingPairs,
        direction,
      ...
      await this.validationService.validateAsyncApiMappingComplete(
        source,
        targets,
        mapping,
        direction
      ...
      const downloadLink = await this.adapterService.createAsyncApiAdapter(
        mapping,
  ...
  public async getAllAttributeSuggestions() {
    ...
    if (publish) {
      ...
      for (const [key, value] of Object.entries(targets || {})) {
        request[key] = await getMessageSchema(value.api, {
    ...
    for (const attributeId of attributes) {
      const component = await this.attributeMappingService.getComponentLocal(
        attributeId,
        this.mappingPairs
      );
      ...
      if (publish) {
        ...
        relevant = component.filter(
          (n) =>
            targets.some((t) => n.startsWith(t)) &&
            !this.mappingPairs.some((m) => m.required.join(".") === n)
        );
      } else {
        const source = this.parseSource();
      ...
      const suggestedMappingPairs = (
        await Promise.all(
          relevant.map(async (node) => {
            return [
              node,
              await this.attributeMappingService.getMappingLocal(
                attributeId,
                node,
                this.mappingPairs
              ),
  ...
  public async finishMapping() {
    if (!this.inputForm.get("strict").value) {
      return;
    }
    ...
    try {
      ...
      const mappingPairs = this.mappingPairs.filter((mp) =>
        this.attributeMappingService.validateMapping(mp)
      );
      for (const m of mappingPairs) {
        await this.attributeMappingService.addMapping(
          m.provided[0].join("."),
          m.required.join("."),
          m.mappingCode
        );
  ...
  private showSuccessDialog() {
    const dialogRef: MatDialogRef<GenericDialog, void> = this.dialog.open(
      GenericDialog,
      {
        position: {
          top: "5%",
    ...
    const dialogRef: MatDialogRef<GenericDialog, void> = this.dialog.open(
      GenericDialog,
      {
        ...
        data: {
          title: "Mapping Error",
          content:
            "An error occured while saving the mapping. This is most likely because the already exists an identical mapping. However, if you keep experiencing this issue, please contact us.",
          buttons: [ButtonType.OK],
        },
      }
    );
